...
export default function ResetCode() {
  ...
  let validationSchema = Yup.object().shape({
    resetCode: Yup.string()
      .matches(/^[0-9]{6}$/, "Code must be valid 6 number")
          ...
          <section className="px-2 py-12">
            ...
            <form onSubmit={formikCode.handleSubmit} className="select-none">
              <div className="mb-6">
                ...
                <button
                  disabled={
                    !formikCode.isValid || !formikCode.dirty ? true : false
                  }
                  onClick={notify}
                  type="submit"
                  className="hover:bg-[#088A08] focus:ring-[#6cce6c]  disabled:opacity-65 disabled:cursor-not-allowed flex-1 w-full px-4 py-2 mx-auto md:w-[75%] block mt-12 font-semibold text-center text-white transition-all duration-300 bg-[#0AAD0A]  rounded-lg cursor-pointer"
                >
                  Verify
                </button>
              </div>
