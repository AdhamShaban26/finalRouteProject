        ...
        <main className="container px-4 mx-auto overflow-hidden">
          <section className="xl:mx-24 px-2 py-12">
            ...
            {data?.data.map((orders) => (
              ...
              >
                <div>
                  {orders.cartItems.map((orderdetail) => (
                    <div
                      key={orderdetail._id}
                    ...
                    >
                      <img
                        className="w-24 mr-3"
                        src={orderdetail.product.imageCover}
                        alt={orderdetail.product.title}
                      />
                      <div>
                        <h3 className="mb-2 text-lg font-medium text-gray-500">
                          {orderdetail.product.title.split(" ", 3).join(" ")}
                        </h3>
                        <h4 className="mb-2 font-bold text-[#0AAD0A]">
                          {orderdetail.price} EGP
                        </h4>
                        <h4>
                          Qty:{" "}
                          <span className="font-medium text-[#0AAD0A]">
                            {orderdetail.count}
                          </span>
                        </h4>
                      </div>
                ...
                <div className="whitespace-nowrap">
                  <h3 className="border-b-[#0AAD0A] w-fit mb-2 text-3xl font-semibold text-gray-900 border-b">
                    Order Details
                  </h3>
                  <div className="flex gap-6 mb-2">
                    ...
                    <p className="text-lg font-medium text-[#0AAD0A]">
                      {orders.id}
                    </p>
                  </div>
                  <div className="flex gap-6 mb-2">
                    ...
                    <p className="text-lg font-medium text-[#0AAD0A]">
                      {capitalizeFirstLetter(orders.paymentMethodType)}
                    </p>
                  </div>
                  <div className="flex gap-6 mb-2">
                    <h4 className="text-lg font-medium text-gray-700">
                    ...
                    <p className="text-lg font-medium text-[#0AAD0A]">
                      {orders?.shippingAddress?.city
                        ? capitalizeFirstLetter(orders?.shippingAddress?.city)
                        : " "}
                    </p>{" "}
                  </div>
                  <div className="flex gap-6 mb-2">
